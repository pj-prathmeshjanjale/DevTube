...
const createVideo = async (req, res) => {
  ...
  try {
    if (thumbnail) {
      // 1. Upload to ImageKit to get a public URL
      ...
      uploadedThumbnailUrl = imagekitResponse.url

      // 2. Send the Public URL to BunnyCDN
      try {
        console.log(`Setting BunnyCDN thumbnail for video ${videoId} to ${uploadedThumbnailUrl}`);
        const bunnyResponse = await axios.post(
          `https://video.bunnycdn.com/library/${process.env.BUNNY_LIBRARY_ID}/videos/${videoId}/thumbnail?thumbnailUrl=${uploadedThumbnailUrl}`, null,
          { headers: { accept: 'application/json', AccessKey: process.env.BUNNY_API_KEY } }
        )
        console.log('BunnyCDN thumbnail response:', bunnyResponse.status);
      } catch (bunnyError) {
        console.error('FAILED to set BunnyCDN thumbnail:', bunnyError.response?.status, bunnyError.response?.data);
        // Do not throw, allow video creation to proceed even if thumbnail fails
      }
...
const getVideo = async (req, res) => {
  try {
    ...
    const { description, title, viewsEnabled, commentsStatus, status, privacySettings, uid, thumbnailUrl } = video
...
const getVideos = async (criteria) => {
  ...
  try {
    ...
    const videosWithDetails = await Promise.all(videos.map(async video => {
      ...
      if (!bunnyVideo) {
        console.warn(`[getVideos] Video ${videoId} not found on BunnyCDN. Skipping.`);
        return null;
      }
