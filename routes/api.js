...
router.get('/getThumbnail', async (req, res) => {
    try {
        ...
        // Check if we have a stored ImageKit URL
        const video = await Video.findOne({ videoId });
        if (video && video.thumbnailUrl) {
            const response = await axios.get(video.thumbnailUrl, {
        ...
        console.error('Error in getThumbnail:', error.message)
        if (error.response) {
            console.error('BunnyCDN Response status:', error.response.status)
            console.error('BunnyCDN Response data:', error.response.data)
        }
